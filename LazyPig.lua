...
LPCONFIG.GREEN = 2                 -- [number or nil] Auto roll on green items
...
function LazyPig_OnUpdate(elapsed)
	...
	if LPCONFIG.GREEN and ctrlstatus and not shiftstatus and altstatus then
...
function LazyPig_Kara40()
	local inInstance, instanceType = IsInInstance()
	if not inInstance or instanceType ~= "raid" or GetNumRaidMembers() <= 10 then
		return false
	end
	local zone = GetRealZoneText()
	return zone == "Tower of Karazhan" or zone == "the rock of desolation"
end
